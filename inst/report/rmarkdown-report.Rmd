...
```{r echo=F, warning=F, fig.width=2.5, fig.height=2}
overall_frame <- data.frame(x = 2, y = 2.5, h = 4.25, w = 4.25,
                            value = params$overall$score, info = "Accuracy score,",
                            info_2 = paste("n = ", params$overall$counts),
...
ggplot(overall_frame, aes(x, y, height = h, width = w, label = info)) +
  ...
  geom_text(color = "white", fontface = "bold", size = 20,
            aes(label = value, x = x - 1.5, y = y + 1.25), hjust = 0) +
  ## Add the labels for each box stored in the `info` column
  geom_text(color = "white", fontface = "bold", size = 10,
            aes(label = info, x = x - 1.5, y = y - 1), hjust = 0) +
  geom_text(color = "white", fontface = "bold", size = 10,
            aes(label = info_2, x = x - 1.5, y = y - 1.45), hjust = 0) +
  coord_fixed() +
